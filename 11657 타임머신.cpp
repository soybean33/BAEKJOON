...
void solution(){
	...
	for(int j=0; j<N; j++){	// N 번 반복
		for(int i=0; i<N; i++){	// N 개의 도시 i 에 대하여
			for(int k=0; k<v[i].size(); k++){	// i 도시에서 갈 수 있는 간선 정보
				Edge next = v[i][k];	// next: 연결되어 있는 간선 정보
				if(nodeTime[i] != 1e18 && nodeTime[next.to] > nodeTime[i] + next.time){	// i 도시에 이미 방문한 적이 있고, 이번에 탐색한 시간이 이전에 기록되어 있는 시간 보다 빠른 경우
...
void input(){
	...
	for(int i=0; i<M; i++){
		int from, to, time;	// from: 시작도시, to: 도착도시, time: 버스를 타고 이동하는데 걸리는 시간
